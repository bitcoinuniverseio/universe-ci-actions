...
export function validateFixtureInputs(inputs) {
  if (!inputs.image.includes("@sha256:")) throw new Error("MySQL image must be pinned by sha256 digest");
...
export async function waitForMysql(platform, container, rootPassword, attempts = 90) {
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    const result = dockerCommand(platform, [
      ...
      `--password=${rootPassword}`, "--silent"
    ], { capture: true, allowFailure: true });
    if (result.status === 0) return;
    await new Promise((resolve) => setTimeout(resolve, 1000));
  }
  throw new Error(`MySQL fixture ${container} did not become healthy`);
}
